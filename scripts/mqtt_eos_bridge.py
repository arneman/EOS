...
EOS_PV_FORECAST_INTERVAL_S = 900  # How often to refresh PV prediction (15 min)
MQTT_KEEPALIVE_S = 60
    ...
    while repeat_thread_running:
        try:
            ...
            if mode in CHARGING_MODES:
                battery_power_w = int(
                    row.get("genetic_ac_charge_factor", 0.0) * REAL_BATTERY_MAX_CHARGE_W
                )
            else:
                battery_power_w = 0
            ...
            for i, ts in enumerate(sorted_timestamps):
                ...
                if m in CHARGING_MODES:
                    power_w = int(
                        r.get("genetic_ac_charge_factor", 0.0)
                        * REAL_BATTERY_MAX_CHARGE_W
                    )
                elif m in DISCHARGING_MODES:
                    power_w = -int(
                        r.get("genetic_discharge_allowed_factor", 0.0)
                        * REAL_BATTERY_MAX_CHARGE_W
                    )
                else:
                    power_w = 0
