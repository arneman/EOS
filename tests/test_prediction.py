...
def test_provider_sequence(prediction):
    ...
    assert isinstance(prediction.providers[6], FeedInTariffMarketPrice)
    ...
    assert isinstance(prediction.providers[13], PVForecastImport)
    assert isinstance(prediction.providers[14], WeatherBrightSky)
    assert isinstance(prediction.providers[15], WeatherClearOutside)
    assert isinstance(prediction.providers[16], WeatherOpenMeteo)
    assert isinstance(prediction.providers[17], WeatherImport)
...
def test_prediction_repr(prediction):
    ...
    assert "FeedInTariffMarketPrice" in result
